...
#define DM250US_VOP_BASE		0x1010e000
#define DM250US_VOP_SYS_CTRL		0x00
#define DM250US_VOP_AXI_BUS_CTRL		0x2c
#define DM250US_VOP_REG_CFG_DONE		0x90
#define DM250US_VOP_WIN0_EN		BIT(0)
#define DM250US_VOP_STANDBY		BIT(30)
#define DM250US_VOP_OUTPUT_CLK_EN		(BIT(26) | BIT(24) | BIT(22))
#define DM250US_GRF_LVDS_CON0		0x20008150
#define DM250US_LVDS_CON0_WMSK		0x03cf
 ...
 */
static void dm250us_display_shutdown(void)
{
	...
	if (!ret) {
		...
		if (ret)
			printf("DM250US: failed to turn off backlight (%d)\n", ret);
	} else {
		printf("DM250US: backlight unavailable during shutdown (%d)\n", ret);
	}

	/* Stop the active window and request VOP standby. */
	val = readl(DM250US_VOP_BASE + DM250US_VOP_SYS_CTRL);
	val &= ~DM250US_VOP_WIN0_EN;
	val |= DM250US_VOP_STANDBY;
	writel(val, DM250US_VOP_BASE + DM250US_VOP_SYS_CTRL);
	writel(1, DM250US_VOP_BASE + DM250US_VOP_REG_CFG_DONE);

	/* Allow the pending shadow-register update to reach a frame boundary. */
	mdelay(20);

	val = readl(DM250US_VOP_BASE + DM250US_VOP_AXI_BUS_CTRL);
	val &= ~DM250US_VOP_OUTPUT_CLK_EN;
	writel(val, DM250US_VOP_BASE + DM250US_VOP_AXI_BUS_CTRL);
	...
	if (!ret) {
		...
		if (!ret)
			...
			printf("DM250US: LVDS PHY unavailable during shutdown (%d)\n",
			       ret);
	} else {
		printf("DM250US: LVDS display unavailable during shutdown (%d)\n",
		       ret);
	}

	/* GRF writes use the upper half-word as a write mask. */
	writel(DM250US_LVDS_CON0_WMSK << 16, DM250US_GRF_LVDS_CON0);
	if (have_dphy) {
		ret = generic_phy_power_off(&dphy);
		if (ret)
			printf("DM250US: failed to power off LVDS PHY (%d)\n", ret);
	}

	puts("DM250US: display disabled before legacy U-Boot\n");
}
