...
/* DM250US hardware setup; US-specific wiring is described in the DTS. */
#include <config.h>
#include <button.h>
#include <dm.h>
#include <env.h>
...
int rk_board_late_init(void)
{
	...
	if (pressed("Right Shift") && pressed("Left Alt") && pressed("Power")) {
		env_set("dm250us_recovery", "1");
		puts("DM250US: recovery keys held (used by dm250usboot emmc)\n");
	}

	ret = uclass_get_device_by_name(UCLASS_PMIC, "pmic@1c", &pmic);
	if (ret) {
		printf("DM250US: RK818 unavailable (%d)\n", ret);
		return 0;
	}
...
pmic_error:
	printf("DM250US: RK818 setup failed (%d)\n", ret);
	return 0;
}
